...
pygame.display.init()
pygame.font.init()
pygame.display.set_caption("Fluid physics attempt")
...
world = PhysicsWorld()
world.add(
    RigidBody(screen_size[0],50,screen_size[0]/2,screen_size[1]-50,mass=inf)
    ...
    for event in pygame.event.get():
        if event.type == QUIT:
            ...
            body = RigidBody(
                ...
                angle=random.randint(0, 90),mass=1
            )
            world.add(body)
            body.add_world_force(gravity,Vector2D(body.width/2,body.height/2))
